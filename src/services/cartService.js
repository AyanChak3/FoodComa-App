...
async function modifyCart(userId,productId,shouldAdd = true){ //shouldAdd = true then we will add quantity to the item
    const quantityValue = (shouldAdd == true)? 1 : -1;
    const cart = await getCart(userId) //fetches the cart
    const product = await getProductById(productId); //fetches the product
    if(!product){
    ...
    //May be the product is already in the cart. Then we will only increase its quantity
    ...
    await cart.save();//changes made to cart are saved
    return cart;
}
